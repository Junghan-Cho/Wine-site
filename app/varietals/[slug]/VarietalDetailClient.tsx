...
export default function VarietalDetailClient({ slug }: { slug: string }) {
  ...
  return (
    <section className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1.2fr)] md:items-start">
        <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
          {varietal.imageUrl ? (
            ...
            <img src={varietal.imageUrl} alt={varietal.nameEn} className="h-56 w-full object-cover md:h-64" />
          ) : (
        ...
        <div className="space-y-4">
          <div>
            <h1 className="font-display text-2xl text-slate-50">{dv.name}</h1>
          ...
          <p className="text-sm text-slate-200">{dv.oneLiner}</p>
          <div className="space-y-1 text-sm text-slate-300">
            <h2 className="font-semibold text-slate-100">{t('taste_aroma')}</h2>
            <p className="text-sm text-slate-300">{dv.tasteAndAroma}</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {varietal.pairings.map((p) => (
              <span key={p} className="rounded-full bg-slate-900 px-3 py-1 text-slate-100">
                {p}
              </span>
            ))}
          </div>
      ...
      {winesFromVarietal.length > 0 && (
        <div className="space-y-3">
          <h2 className="font-display text-lg text-slate-50">{t('representative_wines')}</h2>
      ...
      {relatedVarietals.length > 0 && (
        <div className="space-y-3">
          <h2 className="font-display text-lg text-slate-50">{t('related_varietals')}</h2>
          <div className="flex flex-wrap gap-2 text-xs">
