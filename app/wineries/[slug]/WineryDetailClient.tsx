...
export default function WineryDetailClient({ slug }: { slug: string }) {
    ...
    <section className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-[minmax(0,1.1fr)_minmax(0,1.2fr)] md:items-start">
        <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
          {winery.imageUrl ? (
            ...
            <img src={winery.imageUrl} alt={winery.nameEn} className="h-56 w-full object-cover md:h-64" />
          ) : (
        ...
        <div className="space-y-4">
          <div>
            <h1 className="font-display text-2xl text-slate-50">{wineryDisplay.name}</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            {wineryDisplay.classification && (
              <span className="rounded-full border border-slate-700 px-2 py-1 text-slate-200">{wineryDisplay.classification}</span>
            )}
          </div>
          <p className="text-sm text-slate-200">{wineryDisplay.oneLiner}</p>
          ...
          {winery.website && (
            <a href={winery.website} target="_blank" rel="noreferrer" className="inline-flex items-center text-xs text-accent hover:underline">
      ...
      {wineryWines.length > 0 && (
        <div className="space-y-3">
          <h2 className="font-display text-lg text-slate-50">{t('wines_from_producer')}</h2>
                ...
                >
                  {w.imageUrl && (
                    <div className="mb-2 h-24 overflow-hidden rounded-md bg-slate-900">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={w.imageUrl} alt={w.nameEn} className="h-full w-full object-cover" />
                    </div>
                  )}
                  ...
                  <div className="text-[11px] text-slate-400">
                    ...
                    {TYPE_LABEL_KEYS[w.type] ? t(TYPE_LABEL_KEYS[w.type]) : w.type}
                  </div>
                </Link>
              )
            })}
